...
from rnatargeting.models.layers import recurrent_dense, strided_down, encoder_down_block


def guide_nolin_ninef_model(guidelength, num_strided_down=4, kernel=3, cnn_units=32, dense_units=8, recurrent_layers=1, noise=True):
    #seq = keras.Input(shape=(30, 4))
    seq = keras.Input(shape=(guidelength, 4))
...
def guide_nolin_threef_model(guidelength, num_strided_down=4, kernel=3, cnn_units=32, dense_units=8, recurrent_layers=1, noise=True):
    #seq = keras.Input(shape=(30, 4))
    seq = keras.Input(shape=(guidelength, 4))
